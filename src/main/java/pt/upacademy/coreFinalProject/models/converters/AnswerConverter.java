...
public class AnswerConverter extends EntityConverter<Answer, AnswerDTO>{
	...
	public Answer toEntity(AnswerDTO dto) {
		return new Answer(dto.getId(), ANSWER_QUESTIONNAIRE_SERVICE.get(dto.getAnsweredQuestionaireId()), dto.getAnswer(), dto.getQuestionId());
	}

	@Override
	public AnswerDTO toDTO(Answer entity) {
		return new AnswerDTO(entity.getId(), entity.getAnsweredQuestionaire().getId(), entity.getAnswer(), entity.getQuestionId());
	}
